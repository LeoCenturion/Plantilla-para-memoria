...
\begin{itemize}
    ...
    \item Disminución Media de Exactitud (MDA): evalúa la importancia de una característica permutando aleatoriamente sus valores y midiendo la caída resultante en el rendimiento del modelo. Una caída significativa indica que la característica es importante.
    \item 	Importancia de Característica Individual (SFI): entrena y evalúa un modelo utilizando una sola característica a la vez para medir su poder predictivo de forma aislada.
...
\begin{itemize}
    ...
    \item 	Etiquetado dinámico: imilar al anterior, pero los umbrales para definir las etiquetas se ajustan en función de la volatilidad reciente del mercado.
...
\begin{itemize}
    ...
    \item 	Probabilistic Sharpe Ratio (PSR): corrige el Ratio de Sharpe tradicional y lo penaliza si los retornos tienen sesgo negativo (skewness), colas pesadas (kurtosis) o si el historial de datos es demasiado corto. Proporciona la probabilidad real de que el Sharpe de la estrategia sea mayor que un valor de referencia (benchmark).
