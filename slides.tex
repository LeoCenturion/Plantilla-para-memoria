...
\AtBeginDocument{\setbeamertemplate{headline}{}}
...
%----------------------------------------------------------------------------------------
% Paleta de colores (versiones claras para legibilidad en fondo oscuro)
%----------------------------------------------------------------------------------------
\definecolor{fiubablue}{HTML}{003087}
\definecolor{accent}{HTML}{E8501A}
\definecolor{coltrading}{HTML}{9B9B9B}   % gris claro  — trading clásico
\definecolor{colstat}{HTML}{E59866}      % naranja claro — estadístico
\definecolor{colml}{HTML}{5DADE2}        % azul claro   — machine learning
\definecolor{coltrans}{HTML}{58D68D}     % verde claro  — transformer

%--- Gotham theme primary/accent colors ---
\colorlet{gotham@primaryA}{fiubablue}
\colorlet{gotham@primaryB}{fiubablue!70!black}
\colorlet{gotham@secondary}{accent}

\setbeamercolor{alerted text}{fg=accent}
\setbeamercolor{section in toc}{fg=accent}

%----------------------------------------------------------------------------------------
% tcolorbox styles (adaptados para fondo oscuro)
%----------------------------------------------------------------------------------------
\tcbset{
  kpibox/.style={
    enhanced, colback=#1!25!black, colframe=#1!80,
    boxrule=1pt, arc=4pt,
    left=6pt, right=6pt, top=4pt, bottom=4pt,
    fontupper=\bfseries\color{white},
  },
  familybox/.style={
    enhanced, colback=#1!20!black, colframe=#1!70,
    boxrule=2pt, arc=3pt,
...
% Colored family tag
\newcommand{\famtag}[2]{%
  \tikz[baseline=-0.5ex]\node[fill=#1!30!black, draw=#1!80, rounded corners=2pt,
    inner sep=2pt, font=\footnotesize\bfseries, text=white]{#2};%
}

%----------------------------------------------------------------------------------------
\title{Predicción de series de tiempo aplicado\\al trading de criptomonedas usando\\la arquitectura de Transformers}
...
\begin{frame}[plain, noframenumbering]
  ...
  \begin{columns}[T]
    \begin{column}{0.78\textwidth}
      \begin{beamercolorbox}[wd=\textwidth]{title}
        {\usebeamerfont{title}\fontsize{16}{20}\selectfont\inserttitle\par}
      \end{beamercolorbox}
      \vspace{0.2em}
      \begin{beamercolorbox}[wd=\textwidth]{subtitle}
        {\usebeamerfont{subtitle}\fontsize{11}{14}\selectfont\insertsubtitle\par}
      \end{beamercolorbox}
    \end{column}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      {\footnotesize\textcolor{white!70}{\textbf{Alumno}}}\\[0.3em]
      Ing. Martín Leonardo Centurión\\[0.6em]
      {\footnotesize\textcolor{white!70}{\textbf{Director}}}\\[0.3em]
      Dr. Camilo Argoty\\[0.8em]
      {\small Junio de 2026}
    \end{column}
    \begin{column}{0.48\textwidth}
      {\footnotesize\textcolor{white!70}{\textbf{Jurados}}}\\[0.3em]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[scale=1.00]
        % ── INPUT BOX ──────────────────────────────
        \draw[draw=fiubablue!60, fill=fiubablue!25!black, rounded corners=4pt]
          (0,0) rectangle (4.5,1.85);
        \node[font=\tiny\bfseries, fiubablue!70] at (2.25,1.70)
          {Datos de entrada};
        % Price line
        ...
        \foreach \x/\h in {0.30/0.20, 0.60/0.25, 0.85/0.13,
          ...
          \fill[colml!60, opacity=0.8]
            (\x-0.05,0.05) rectangle (\x+0.05, 0.05+\h*0.5);
        }
        \node[font=\tiny, fiubablue!70] at (2.25,0.17)
          {RSI · MACD · Bollinger · ADX};

        % ── ARROW ──────────────────────────────────
        \draw[-{Stealth}, thick, fiubablue!60] (2.25,-0.05) -- (2.25,-0.42);

        % ── TRANSFORMER BOX ────────────────────────
        \draw[draw=coltrans!70, fill=coltrans!20!black, rounded corners=4pt]
          (0.3,-0.48) rectangle (4.2,-1.88);
        \node[font=\tiny\bfseries, coltrans!80] at (2.25,-0.63)
          {Transformer — Chronos-2};
        % Attention blocks
        \foreach \i in {0,1,2,3,4} {
          \draw[draw=coltrans!65, fill=coltrans!30!black, rounded corners=2pt]
            (0.50+\i*0.68,-0.85) rectangle (1.08+\i*0.68,-1.55);
          \node[font=\tiny, coltrans!80] at (0.79+\i*0.68,-1.20) {A};
          \ifnum\i<4
            \draw[->, coltrans!55, very thin]
              (1.08+\i*0.68,-1.20) -- (1.16+\i*0.68,-1.20);
          \fi
        }
        \node[font=\tiny, coltrans!75] at (2.25,-1.73)
        ...
        % ── DECISION BOXES ─────────────────────────
        \draw[draw=coltrans!70, line width=1pt, fill=coltrans!25!black, rounded corners=4pt]
          (0.0,-2.35) rectangle (2.15,-2.85);
        \node[font=\normalsize\bfseries, coltrans!90] at (1.07,-2.60)
          {↑\ BTC};

        \draw[draw=coltrading!70, line width=1pt, fill=coltrading!25!black, rounded corners=4pt]
          (2.35,-2.35) rectangle (4.50,-2.85);
        \node[font=\normalsize\bfseries, coltrading!90] at (3.42,-2.60)
          {=\ USDT};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \begin{tikzpicture}[scale=0.75]
        %── LSTM ──────────────────────────────────────
        \node[font=\scriptsize\bfseries, colstat!90, anchor=west]
          at (0,5.35) {RNN / LSTM — secuencial};
        \foreach \i/\x in {1/0.0,2/0.9,3/1.8,4/2.7} {
          \draw[draw=colstat!60, fill=colstat!25!black, rounded corners=2pt]
            (\x,4.58) rectangle (\x+0.72,5.03);
          \node[font=\tiny, colstat!90] at (\x+0.36,4.805) {$h_{\i}$};
        }
        \foreach \x in {0.0,0.9,1.8} {
          \draw[-{Stealth},thin,colstat!55] (\x+0.72,4.805)--(\x+0.9,4.805);
        }
        \draw[accent!70, dashed, -{Stealth}]
          (0.36,4.58) to[bend right=18] (3.06,4.58);
        \node[font=\tiny, accent!90, anchor=west]
          at (3.15,4.80) {\faExclamationTriangle\ degradada};

        %── Transformer ───────────────────────────────
        \node[font=\scriptsize\bfseries, coltrans!90, anchor=west]
          at (0,3.90) {Transformer — autoatención};
        \foreach \i/\x in {1/0.0,2/0.9,3/1.8,4/2.7} {
          \draw[draw=coltrans!65, fill=coltrans!25!black, rounded corners=2pt]
            (\x,2.60) rectangle (\x+0.72,3.05);
          \node[font=\tiny, coltrans!90] at (\x+0.36,2.825) {$x_{\i}$};
        }
        ...
        \node[font=\tiny, coltrans!90, anchor=west]
          at (3.15,2.825) {\faCheckCircle\ global};

        %── Summary ───────────────────────────────────
        \node[draw=coltrans!55, fill=coltrans!20!black, rounded corners=3pt,
              font=\tiny\bfseries, text=coltrans!90,
...
\begin{frame}{Aprendizaje supervisado}
  \begin{columns}[T]
    ...
    \begin{column}{0.62\textwidth}
      \includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{hyperparameter_pipeline.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Etiquetado y validación sin data leakage}
  \begin{columns}[T]
    ...
    \begin{column}{0.60\textwidth}
      \includegraphics[width=\textwidth, height=0.72\textheight, keepaspectratio]{triple-barrier.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Pipeline de datos}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.62\textwidth}
      \includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{preprocessing_pipeline.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Barras de volumen: Muestreo adaptativo}
  \begin{columns}[T]
    ...
    \begin{column}{0.60\textwidth}
      \centering
      \includegraphics[width=\textwidth, height=0.78\textheight, keepaspectratio]{bars-volume-1m.png}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.53\textwidth}
      ...
      \begin{tikzpicture}[scale=0.92,
        ...
        %── Orquestador ───────────────────────────
        \node[box, draw=fiubablue!60, fill=fiubablue!25!black,
              minimum width=4.4cm] (orch) at (2.2, 4.0)
          {\faCogs\ Orquestador};

        %── Motor ─────────────────────────────────
        \node[box, draw=fiubablue!50, fill=fiubablue!20!black,
              minimum width=4.4cm] (motor) at (2.2, 3.0)
          {\faRobot\ Motor del bot};
        \draw[arr, fiubablue!60] (orch) -- (motor);

        %── Exchange (left) ───────────────────────
        \node[box, draw=colml!70, fill=colml!20!black] (exch) at (0.8, 1.75)
          {\faDatabase\ Exchange};
        % Arrow from left side of Motor bottom
        \draw[arr, fiubablue!55]
          ($(motor.south)+(-0.85,0)$) -- (exch.north);

        %── Estrategia (right) ────────────────────
        \node[box, draw=coltrans!70, fill=coltrans!20!black] (strat) at (3.8, 1.75)
        ...
        %── BTC/USDT ──────────────────────────────
        \node[box, draw=accent!80, fill=accent!20!black] (mkt) at (0.8, 0.60)
          {BTC/USDT};
        \draw[arr, colml!70] (exch) -- (mkt);
        \draw[arr, accent!60, bend right=40] (mkt) to (exch);

        %── Señal: Estrategia → Motor (right side) ─
        \draw[arr, coltrans!70]
          (strat.north)
        -- ($(motor.south)+(+0.85,0)$);
        \node[font=\small\bfseries, coltrans!85, anchor=west]
    ...
    \begin{column}{0.55\textwidth}
      \begin{tikzpicture}
        \begin{axis}[
          ...
          bar width=0.45cm,
          xlabel={\small F1 OOS},
          xlabel style={font=\small, color=white},
          ytick={1,2,3,4},
          yticklabels={
          ...
          xticklabel style={font=\small, color=white},
          nodes near coords,
  ...
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      ...
      \begin{tabular}{cc}
        ...
        2 & 0,8235 \\
        3 & 0,8221 \\
        \rowcolor{coltrans!25!black} 4 & \textbf{0,8303} \\
        \rowcolor{coltrans!25!black} 5 & \textbf{0,8277} \\
        \midrule
...
\begin{frame}{Conclusiones y próximos pasos}
  \begin{columns}[T]
    ...
    \begin{column}{0.59\textwidth}
      \centering
      \includegraphics[width=\textwidth, height=0.55\textheight, keepaspectratio]{bloques-tp-final.drawio.png}\\[0.3em]
      \footnotesize\raggedright
      \textbf{\faArrowRight\; Próximos pasos:}
      Bet sizing · Ensamble · HRP · Microestructura.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Demostración: Bot en producción}
  ...
  \begin{tcolorbox}[familybox=coltrans, top=8pt, bottom=8pt,
    ...
    \href{https://drive.google.com/file/d/1ybZkajjzUriHRqoBw0tH0f-fCc4mB_6i/view?usp=sharing}{%
      \small\ttfamily https://drive.google.com/file/d/1ybZkajjzUriHRqoBw0tH0f-fCc4mB\_6i/view?usp=sharing}
  \end{tcolorbox}
  \vspace{0.7em}
  \begin{columns}[T]
